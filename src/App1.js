...
function OrderForm({ order, clients = [], onSave, onClose }) {
  ...
  const handlePdfImport = async (file) => {
    const url = URL.createObjectURL(file);
    // Stocker le PDF immédiatement dans le formulaire (objet {name, url})
    setForm(f => ({ ...f, devis_pdf: { name: file.name, url } }));
    try {
      ...
      setForm(f => ({ ...f,
        ...
        // Le PDF est déjà stocké, on s'assure qu'il reste bien là
        devis_pdf: { name: file.name, url },
      }));
    } catch (e) {
      console.error("Analyse PDF échouée", e);
    }
  };
...
const s = {
  root: { minHeight: "100vh", background: "#F5F4F0", color: "#1A1D2C", fontFamily: "'DM Sans', 'Segoe UI', sans-serif", paddingBottom: 48 },

  // Header
  header: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 28px", borderBottom: "1px solid #E2DDD6", flexWrap: "wrap", gap: 12, background: "#FDFCFA" },
  headerLeft: { display: "flex", alignItems: "center", gap: 14 },
  logo: { fontSize: 30, color: "#4F9CF9" },
  appName: { fontSize: 21, fontWeight: 900, letterSpacing: 3, color: "#1A1D2C" },
  appSub: { fontSize: 10, color: "#9A8F80", letterSpacing: 1.5, textTransform: "uppercase", marginTop: 2 },
  ...
  // Column
  col: { flex: "0 0 270px", background: "#EEEAE4", borderRadius: 14, border: "1.5px solid #DDD8D0", display: "flex", flexDirection: "column", transition: "border-color .2s, background .2s" },
  colHead: { display: "flex", alignItems: "center", gap: 8, padding: "14px 14px 4px" },
  colTitle: { flex: 1, fontWeight: 700, fontSize: 12, letterSpacing: 1, textTransform: "uppercase", color: "#5A5247" },
  ...
  empty: { textAlign: "center", color: "#B0A898", fontSize: 12, padding: "20px 0", border: "1.5px dashed #C8C0B5", borderRadius: 10 },

  // Card
  card: { background: "#FDFCFA", border: "1px solid #E2DDD6", borderRadius: 11, padding: "11px 13px", cursor: "grab", userSelect: "none" },
  ...
  cardRef: { flex: 1, fontSize: 10, color: "#9A8F80", fontWeight: 700, letterSpacing: 1 },
  cardMontant: { fontSize: 14, fontWeight: 800 },
  cardClient: { fontWeight: 800, fontSize: 15, color: "#1A1D2C", marginBottom: 6 },
  ...
  fileChip: { fontSize: 10, color: "#4F9CF9", padding: "3px 7px", background: "#4F9CF918", borderRadius: 5, fontWeight: 700 },
  cardNote: { fontSize: 11, color: "#9A8F80", borderTop: "1px solid #E2DDD6", paddingTop: 6, marginTop: 6, fontStyle: "italic" },
  cardDate: { fontSize: 10, color: "#B0A898", marginTop: 5 },

  // Modal
  overlay: { position: "fixed", inset: 0, background: "rgba(0,0,0,.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(4px)" },
  modal: { background: "#FDFCFA", borderRadius: 18, border: "1px solid #E2DDD6", width: "92%", maxHeight: "90vh", overflowY: "auto", position: "relative" },
  modalTop: { padding: "20px 22px 14px", borderBottom: "2px solid", position: "relative" },
  modalRef: { fontSize: 10, color: "#9A8F80", fontWeight: 700, letterSpacing: 2, textTransform: "uppercase" },
  modalClient: { fontSize: 22, fontWeight: 900, color: "#1A1D2C", marginTop: 4 },
  closeBtn: { position: "absolute", top: 16, right: 16, background: "none", border: "none", color: "#9A8F80", fontSize: 18, cursor: "pointer" },
  modalBody: { padding: "16px 22px" },
  modalFoot: { padding: "12px 22px 20px", display: "flex", alignItems: "center", borderTop: "1px solid #E2DDD6" },

  // Info grid
  infoGrid: { display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 },
  infoCell: { background: "#EEEAE4", borderRadius: 10, padding: "10px 12px" },
  infoCellLabel: { fontSize: 10, color: "#9A8F80", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 },
  infoCellValue: { fontSize: 14, fontWeight: 700, color: "#1A1D2C" },

  // Sections
  section: { marginBottom: 16, background: "#EEEAE4", borderRadius: 10, padding: "12px 14px" },
  sectionTitle: { fontSize: 11, color: "#7A6F62", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10, fontWeight: 700 },
  toggleRow: { display: "flex", justifyContent: "space-between", alignItems: "center" },
  toggleLabel: { fontSize: 13, color: "#3A3530" },
  toggle: { border: "none", borderRadius: 8, padding: "6px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer" },

  filesLabel: { fontSize: 11, color: "#9A8F80", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 },
  fileChipFull: { display: "block", color: "#4F9CF9", fontSize: 12, textDecoration: "none", padding: "6px 10px", background: "#EEF4FE", borderRadius: 7, marginBottom: 5 },
  uploadBtn: { background: "none", border: "1.5px dashed #C8C0B5", color: "#9A8F80", borderRadius: 8, padding: "7px 14px", fontSize: 12, cursor: "pointer", marginTop: 4, width: "100%" },

  moveBtn: { background: "none", border: "1.5px solid", borderRadius: 8, padding: "6px 13px", fontSize: 12, cursor: "pointer", fontWeight: 700 },

  // Buttons
  btnEdit: { background: "#EEEAE4", color: "#3A3530", border: "1px solid #DDD8D0", borderRadius: 8, padding: "8px 16px", fontSize: 13, cursor: "pointer", fontWeight: 700 },
  ...
  fieldLabel: { display: "block", fontSize: 10, color: "#9A8F80", textTransform: "uppercase", letterSpacing: 1, marginBottom: 5 },
  ...
  suggestionItem: { padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid #EEEAE4" },
  suggestionNom: { fontWeight: 700, color: "#1A1D2C", fontSize: 14 },
  suggestionInfos: { color: "#4F9CF9", fontSize: 11, marginTop: 2 },

  // Clients book
  clientRow: { display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 14px", background: "#EEEAE4", borderRadius: 10, marginBottom: 8, border: "1px solid #DDD8D0" },
  clientNom: { fontWeight: 800, color: "#1A1D2C", fontSize: 15, marginBottom: 4 },
  clientInfos: { display: "flex", flexWrap: "wrap" },
  clientLink: { color: "#4F9CF9", fontSize: 12, textDecoration: "none", fontWeight: 600 },
  clientAxo: { fontSize: 10, color: "#9A8F80", marginTop: 4, letterSpacing: 1 },
  clientCmdBadge: { background: "#4F9CF918", color: "#4F9CF9", borderRadius: 6, padding: "2px 8px", fontSize: 10, fontWeight: 700 },
  ...
  clientDel: { background: "none", border: "none", color: "#B0A898", fontSize: 13, cursor: "pointer" },

  // SAV
  savBadgeCard: { fontSize: 10, color: "#E04040", background: "#E0404015", border: "1px solid #E0404030", borderRadius: 5, padding: "3px 7px", marginTop: 6, fontWeight: 700, display: "inline-block" },
  savComment: { background: "#FDF0F0", border: "1px solid #E0404022", borderRadius: 8, padding: "10px 12px", marginBottom: 8 },
  savCommentText: { color: "#5A2020", fontSize: 13, lineHeight: 1.5, marginBottom: 6 },
  savCommentMeta: { display: "flex", justifyContent: "space-between", alignItems: "center" },
  savCommentDate: { fontSize: 10, color: "#9A8F80", letterSpacing: 0.3 },
  savCommentDel: { background: "none", border: "none", color: "#B0A898", fontSize: 13, cursor: "pointer", padding: "0 4px" },
  btnSavAdd: { background: "#E0404015", color: "#E04040", border: "1.5px solid #E0404040", borderRadius: 8, padding: "7px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer", width: "100%" },

  // Contact
  contactRow: { display: "flex", gap: 10, flexWrap: "wrap" },
  contactBtn: { display: "inline-flex", alignItems: "center", gap: 6, background: "#4ECBA115", color: "#3BAB85", border: "1px solid #4ECBA133", borderRadius: 8, padding: "8px 14px", fontSize: 13, fontWeight: 700, textDecoration: "none", cursor: "pointer" },
  contactAlert: { marginTop: 10, background: "#4ECBA115", border: "1px solid #4ECBA133", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#3BAB85", fontWeight: 600 },
  cardCallBtn: { display: "block", marginTop: 7, background: "#4ECBA115", color: "#3BAB85", border: "1px solid #4ECBA133", borderRadius: 7, padding: "5px 10px", fontSize: 11, fontWeight: 700, textDecoration: "none", textAlign: "center" },
  ...
  aiBadge: { background: "#7B6EF615", color: "#7B6EF6", border: "1px solid #7B6EF633", borderRadius: 6, padding: "4px 10px", fontSize: 10, fontWeight: 700, letterSpacing: 0.5, marginBottom: 6, display: "inline-block" },
